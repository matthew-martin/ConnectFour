 ...
 */
public final class GameController_test
{
	/// State of the game.
	 ...
	 */
	public GameController_test(final GameStateModule game, final IOModule io, final AIModule[] players, final int AI_time)
	{
	...
	public void play()
	{
		...
		// io.drawBoard(game); // Uncomment to draw the board.
		// While not finished
	...
	private void tryMakeMove(final int move)
	{
		...
		if(!game.canMakeMove(move))
		{
			...
			// System.out.println("Cannot make move " + move + " on board:");
			// IOModule io = new TextDisplay();
			// io.drawBoard(game);
			...
			for(int j = 0; j < game.getWidth(); j++)
				...
				{
					game.makeMove(j);
					System.err.println("Player " + activePlayer + " Attempted to play illegal move " + move +
