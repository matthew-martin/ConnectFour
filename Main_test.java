...
//		                }:q!:q:
		...
		{
			 // Every 25 lines, print the header.
			if (i % 25 == 0)
			{
				System.out.println("                RESULTS    |  P1 Wins  |  P2 Wins  |    Draws  |  P1 Win % |  P2 Win % |    Draw % |");
			}
			...
			GameController_test controller = new GameController_test(game, io, players, AI_time);
			controller.play();
			...
			{
				System.out.print("Player " + game.getWinner() + " won");
				if (game.getWinner() == 1)
				{
					p1W++;
				}
			...
			IOModule textDisp = new TextDisplay();
			textDisp.drawBoard(game);
			int totalGames = p1W + p2W + draw;
			float p1WPercent = (float)p1W * 100.0f / (float)totalGames;
			float p2WPercent = (float)p2W * 100.0f / (float)totalGames;
			float drawPercent = (float)draw * 100.0f / (float)totalGames;
			
			System.out.format("    %9d  |%9d  |%9d  |%9d  |%9.2f%% |%9.2f%% |%9.2f%% |",
				totalGames, p1W, p2W, draw, p1WPercent, p2WPercent, drawPercent);
			System.out.println();
		}
	}
}
